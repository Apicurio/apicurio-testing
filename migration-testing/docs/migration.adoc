...
The following table compares the two documented approaches for migrating from {registry-v2} to 3.x.

[cols="1,2,2", options="header"]
|===
| Feature | Zero-Downtime Migration | Traditional Maintenance Window

| *Primary Method*
| Parallel environments with Reverse Proxy/LB routing
| Sequential "Stop-Export-Import-Start" process

| *Complexity*
| **High**: Requires infra for dual-routing and traffic management.
...
| *Infrastructure*
| Requires Nginx/HAProxy + running dual instances.
| Single target environment.

| *Risk Profile*
| Lower user impact; higher configuration complexity.
| Higher user impact; lower technical complexity.

| *Best For*
| High-availability production environments.
| Dev/Staging or systems with flexible SLAs.
|===
...
NOTE:: Regardless of the path chosen, always perform a "dry run" migration in a staging environment to timing the data export/import phases accurately.
...
A key advantage of the parallel deployment (zero-downtime migration) approach is the ability to rollback if issues arise.
