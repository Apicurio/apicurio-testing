...
= Migrating from {registry} {registry-v2} to 3.x

[role="_abstract"]
This documentation provides guidance for migrating from {registry} {registry-v2} to {registry} 3.x.
...
== Overview of changes in {registry} 3.x
...
+
--
{registry} 3.x separates artifact metadata from version metadata and introduces first-class support for empty artifacts. These capabilities enable you to prepare governance metadata, rules, and branch structures before the first version exists. Additional highlights include:
...
The KafkaSQL storage uses a true event journaling pattern that requires a separate Kafka topic with different configuration. The v3 journal format is incompatible with v2.
...
[role="_abstract"]
Select the scenario that best aligns with your architecture and customize the steps for your infrastructure.
...
The objective here is to keep the service reachable. While data is being moved, a reverse proxy (like Nginx) manages the cutover.

.Maintenance window migration
This is the "cleanest" migration from a data integrity standpoint. By stopping all client applications first, you ensure no data is in flight during the move.

NOTE: Regardless of the path chosen, always perform a dry run migration in a staging environment to estimate the timing of the data export and import phases accurately.

During a minimal downtime migration, you temporarily run both {registry} {registry-v2} and 3.x simultaneously. Plan for:
...
Regardless of the path chosen, you also need to consider the following at some point (not required immediately):

* xref:migrating-registry-applications_{context}[Migrating {registry} client applications]
* xref:migrating-registry-configuration_{context}[Updating {registry} configuration]
...
The minimal downtime migration approach uses a load balancer to manage traffic routing, enabling minimal downtime for read operations and a controlled read-only period for write operations during data export.
...
*Impact:* Read operations (schema lookups) continue normally. Write operations (new schemas, updates) return HTTP 405. Kafka producers with auto-register fail to register new schemas.
...
The script supports OAuth2 authentication. Run it interactively or automate it using environment variables.
...
For detailed information about the export and import process, see xref:migrating-registry-data_{context}[Migrating {registry} data].
...
*Downtime:* This operation causes 10-30 seconds of downtime for all operations during reload.
...
Verify that v2 clients can access the 3.x registry through the v2 API compatibility layer. All existing clients continue to operate normally because Registry v3 supports the v2 Core REST API used by v2 clients.
...
. Update client application configurations to point to {registry} 3.x. Clients can continue using the v2 API, which remains available at `/apis/registry/v2`.
. Start client applications.

NOTE: After migration, consider updating client applications to use the v3 API for access to new features. See xref:migrating-registry-applications_{context}[Migrating registry client applications].
...
Export data from v2 and import it into v3 using the export/import APIs.
...
[source,bash]
----
./apicurio-import-regis try-data.sh
----
+
The script:
+
...
You can also use the Registry UI to export from v2 and import into v3.
...
// proc-migrating-registry-applications
...
IMPORTANT: {registry} 3.x maintains backward compatibility with v2 client libraries and SerDes. Continue using v2 clients with a v3 registry during your migration period, allowing gradual client updates.
...
* *v3 SerDes* consumes v2 client Kafka messages (with proper configuration)
...
Kafka SerDes applications are the most common {registry} integration and require careful migration planning. Follow these steps to migrate your SerDes configuration.
...
* *Mixed environment* (v2 and v3 clients) works correctly
...
IMPORTANT: The journal topic must have `cleanup.policy=delete` with infinite retention (`retention.ms=-1`). Do not use log compaction. Preserve all journal events for proper registry operation.
...
* **Journal compaction**: Safely remove older journal events after creating a snapshot
...
NOTE: TLS configuration is identical for v2 and v3. Reuse the same certificates.
...
IMPORTANT: The only *required* UI container property is `REGISTRY_API_URL`. Configure all other UI settings on the application container using `apicurio.ui.*` properties.
...
Find all scripts in the Apicurio Registry GitHub repository under `migration-testing/docs/scripts/`.
...
Scripts work with trusted certificates by default. Use the following setting:
